...
打开 RelayMate 后只需要完成四步：

1. **选择 AI 工具**：选择 Claude 或 Codex。Claude 选项会同时配置 Claude 桌面应用和 Claude Code。
2. **选择中转站**：第一次使用时填写一个容易辨认的平台名称和中转站 URL。以后可以从这里选择已经保存的平台。
3. **输入 API Key**：填写中转站提供的 Key，然后点击“读取模型”。
4. **选择模型**：勾选想在目标软件中看到的模型，再选择一个默认模型，最后点击“测试并应用”。

点击“测试并应用”后的顺序是固定的：先向中转站发一个真实的最小请求做验证，验证通过才备份原文件并写入新配置；验证不通过则**任何文件都不会被修改**，界面上直接显示失败原因。模型步骤如果没有读到列表（有些中转站不开放模型目录），用右下角的“手动添加”直接输入模型名，加进去会自动设为默认模型。

配置 Codex 时，URL 步骤可能会多出一组“让旧对话也用这个中转站”的勾选框，并显示每个 provider 下有多少个历史对话。原因是 Codex 的每个历史对话都记着自己创建时用的 provider **名字**，请求时再按名字去 `config.toml` 里查地址；因此只添加一个新 provider 的话，老对话仍然会往老地址发请求。勾上对应项，RelayMate 会把那些配置段一并改成新地址，老对话就跟着换过来了；这些改动同样在备份范围内，还原时会一起恢复。Claude 没有这个问题——Claude 桌面应用和 Claude Code 都是整个进程读同一份配置，重启后所有对话自动走新地址。

显示“配置完成”后，请完全退出目标 AI 软件再重新打开。只关闭窗口可能不会重新加载配置：
